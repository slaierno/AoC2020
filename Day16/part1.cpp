...
int main() {
    const auto [rules, my_ticket, tickets] = [](){
        ...
        const auto my_ticket = AoC::split(my_ticket_string, '\n')[1];
        const auto make_ticket = [](const auto& s) {
            return AoC::split(s, ',', [](const auto& s) {return std::stoul(s);});
        };
        const auto tickets = AoC::split(AoC::split(tickets_string, ":\n")[1], '\n', make_ticket);
        return input_t{
            rules,
            make_ticket(my_ticket),
            tickets,
        };
    }();
    const auto error_rate = [&rules](const ticket_t& t) {
        const auto has_a_rule = [&rules](const auto v) {
